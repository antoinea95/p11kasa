...
export default function Property() {
    ...
    useEffect(() => {
        propertyChosen && setAccordionData([
            {name: "Description", content: propertyChosen.description, contentType: "paragraph"},
            {name: "Equipements", content: propertyChosen.equipments, contentType: "list"}
        ])
    }, [propertyChosen])
  ...
  return (
    ...
    {propertyChosen && 
        <article className="property">
             {propertyChosen.pictures && <Caroussel pictures={propertyChosen.pictures} />}
             <section className="property-header">
                <div className="property-header_info">
                    <h1 className="property-header_info_title">{propertyChosen.title}</h1>
                    <p className="property-header_info_location">{propertyChosen.location}</p>
                    {propertyChosen.tags && <Tag allTag={propertyChosen.tags} />}
                </div>
               {propertyChosen.host && <div className="property-header_owner">
                    <h2 className="property-header_owner_name">
                        {propertyChosen.host.name}
                        <img className="property-header_owner_picture" src={propertyChosen.host.picture}/>
                    </h2>
